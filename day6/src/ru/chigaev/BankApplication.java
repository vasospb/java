...
public class BankApplication {
    ...
    static void initialize(Bank bank, float[][] initArray) {
        if (bank != null) {
        ...
        bank.clients.get(0).getAccounts().get(0).deposit(initArray[0][0]);
        bank.clients.get(0).getAccounts().get(1).deposit(initArray[0][1]);

        bank.clients.get(1).getAccounts().get(0).deposit(initArray[1][0]);
        bank.clients.get(1).getAccounts().get(1).deposit(initArray[1][1]);
    }
        ...
        for (int i = 0; i < cList.size(); i++) {
            ...
            for (int x = 0; x < aList.size(); x++) {
                ...
                if((initArray[i][x]+depArray[i][x]-withdrawArray[i][x])==aList.get(x).getBallance() ) {
                    System.out.println("\t Account number " +aList.get(x).getAccNumber() +" test Passed");
                } else {
                    System.out.println("\t Account number " +aList.get(x).getAccNumber() +" test failed");
                }
                System.out.println("\t Account number " + aList.get(x).getAccNumber() +" initialy has: "+ initArray[i][x]+
